defmodule ProductAlphaWeb.DefaultmapLive do
 ...
 alias ProductAlpha.Products
  ...
  def handle_event("load-schema", %{"code" => code}, socket) do
    ...
    ll = get_defaultmap_id(ref_prodid)
    IO.inspect(ll.datab)
     #,additional: startwallet()
    {:reply, %{schema: schema(code), defaultdef: startdefault()}, socket}
  end
  ...
  def handle_event("submit", params, socket) do
    ...
    case ExJsonSchema.Validator.validate(ref_schema, params) do
      :ok ->
        ...
        {:ok, _updated_policy} = update_defaultmap(ref_prodid, params)
 ...
 def update_defaultmap(product_id, updated_data) do
  service = get_defaultmap_id(product_id) # Use get_policy!/1 to raise an error if the policy is not found
  IO.inspect(service)
...
def get_defaultmap_id(product_id) do
  ProductAlpha.Recharge.Defaultmap # our scema file
