defmodule ProductAlphaWeb.PolicystateLive do
 ...
 alias ProductAlpha.Products
  ...
  def handle_event("load-schema", %{"code" => code}, socket) do
     ...
     ll = get_policystate_id(ref_prodid)
     IO.inspect(ll.datab)
     #,additional: startwallet()
     #  triggersdef: startvoice()
    {:reply, %{schema: schema(code), policystatedef: ll.datab}, socket}
  end
  ...
  def handle_event("submit", params, socket) do
    ...
    case ExJsonSchema.Validator.validate(ref_schema, params) do
      :ok ->
        ...
        {:ok, _updated_policy} = update_policystatepolicy(ref_prodid, params)
  ...
  def update_policystatepolicy(product_id, updated_data) do
    service = get_policystate_id(product_id) # Use get_policy!/1 to raise an error if the policy is not found
    IO.inspect(service)
  ...
  def get_policystate_id(product_id) do
    ProductAlpha.Policies.Policystate # our scema file
  ...
  def startpolicystate() do
    %{

      "policy_states" => ["High Speed", "RICA", "Device", "PP_OOB"]
  ...
  # or a JSON file, but we'll simply hard-code it for now:
    %{
      ...
      "title" => "Policy State",
      "properties" => %{
        "policy_states" => %{"items" => %{"type" => "string"}, "type" => "array"}
