...
export default {
    mounted() {
      this.pushEvent("load-schema10", { code: "GER" }, (reply, ref) => {
       
        const editor = new JSONEditor(this.el, {
          schema: reply.schema10,
          startval: reply.resdev,
          disable_collapse: false,
          disable_edit_json: false,
          disable_properties: false,
          no_additional_properties: true,
        });
        console.log(reply.schema10);

        // Display the JSON schema
var schemaDisplayTextarea = document.getElementById('schema-display');
schemaDisplayTextarea.value = JSON.stringify(reply.schema10, null, 2);     ///


        document
          .getElementById("json-schema-form-submit10")
          .addEventListener("click", () => {
            ...
            if (errors.length) {
              ...
              this.pushEvent("submit10", editor.getValue());
              console.log("working")
            }
          });
          ...
          // Update Schema button click event
var setSchemaButton = document.getElementById('setschema');
setSchemaButton.addEventListener('click', function() {
  try {
    var updatedSchema = JSON.parse(schemaDisplayTextarea.value);
    schemaDisplayTextarea.value = JSON.stringify(updatedSchema, null, 2);
   // initializeJsonEditor(updatedSchema);
  } catch (error) {
